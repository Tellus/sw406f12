...
$\Gamma_{DecB} = $ & $\mathbf{Character \times Ability \times Behaviour \times Event \times}$ \\
 & $\mathbf{BName \times \mathcal{P}(RGR \times VName \times Integer)}$\\
$\Rightarrow_{DecB}$ & \\
...
\small{\textsc{[Char-Dec]}} \\
 & \large{${\frac{ character, abitity, behaviour, event, sto \; \vdash \langle A_D, R_D, ANames, BName, ENames \rangle \; \Rightarrow \; character', sto'}
 {\langle \texttt{make }  CName \texttt{ from } Character, \; CName, A_D, R_D, ANames, BName, ENames \rangle \Rightarrow character', \;  sto'$}}}\\
 %& \footnotesize{} \\
 \footnotesize{Where} $character'=$ &  \footnotesize{$character[health \mapsto l] [\texttt{next} \mapsto \texttt{new } l, sto \mapsto std]$} \\
 & \footnotesize{$character[mana \mapsto \texttt{new } l][\texttt{next } \mapsto \texttt{new new } l, sto \mapsto std]$} \\
 & \footnotesize{etc.} \\
 \\ \hline
\end{tabular}
...
\emph{\small{Note that the event is basically a boolean expression consisting of a trigger and a condition. As long as both values do not evaluate to true, the event expression is false and the turn continues to point \textbf{3}.}}
...
The calculations are done by a complex algorithm and would turn into a long description, if it was to be described by a semantic transition rule. It is instead described by pseudo-code.
