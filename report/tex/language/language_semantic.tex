...
Given an ability name \textit{AName}, an ability environment must hold information about, targets, effect, cost, and damage. Note that \ac{rgrs} are given within a powerset (written as $\mathcal{P}$).
Therefore the definition of the set of ability environments \textbf{Ability} is:
%
...
return best_action
\end{lstlisting}
The pseudo-code describes how every possible action is simulated, and based at the \texttt{piggy}-value, the best action is chosen and returned.
